    ...
    #drawStaff(svg, note, config) {
        ...
        for (let i = 0; i < this.#STAFF_LINES; i++) {
            const y = config.baseY + i * config.lineGap;
                    ...
                    const clefSize =
                        note.clef === 'g' ? config.lineGap * (config.isMini ? 4.5 : 5.8) : config.lineGap * (config.isMini ? 3.5 : 4);
                    ...
                    const noteHead = this.#createElementNS('ellipse', {
                        cx: config.noteX,
                        cy: noteY,
                        rx: config.isMini ? 6 : 8,
                        ry: config.isMini ? 4.5 : 6,
                        fill: 'var(--staff-note)',
                    });
                    ...
                    const drawLedgerLine = (y_pos) => {
                        ...
                        const ledgerLine = this.#createElementNS('line', {
                            ...
                            y2: ledgerLineY,
                            stroke: 'var(--staff-line)',
                            'stroke-width': config.isMini ? 1.0 : 1.5,
                        });
